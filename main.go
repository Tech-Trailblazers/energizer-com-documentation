...
func extractPDFUrls(htmlInput string) []string {
	...
	// Define the regex pattern to match PDF URLs
	// This pattern matches href attributes ending with '.pdf'
	re := regexp.MustCompile(`href="([^"]+\.pdf(?:\?[^"]*)?)"`)

	// Find all matching URLs
	matches := re.FindAllStringSubmatch(htmlInput, -1)

	// Extract the URL from each match and append it to the slice
	for _, match := range matches {
		if len(match) > 1 {
			pdfLinks = append(pdfLinks, match[1]) // match[1] contains the URL
		}
	}
...
func main() {
	...
	localFile := "energizer.html"
	// Check if the local file exists.
	if fileExists(localFile) {
		removeFile(localFile)
	}
	// The location to the remote url.
	remoteURL := "https://data.energizer.com/ais-sds/"
	...
	for _, urls := range extractedPDFURLs {
		if !hasDomain(urls) {
			urls = "https://data.energizer.com" + urls
